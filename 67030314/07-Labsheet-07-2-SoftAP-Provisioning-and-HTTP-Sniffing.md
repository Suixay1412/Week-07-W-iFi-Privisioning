...
| :------------------------------------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| **1. ชื่อ SoftAP SSID ของ ESP32**                        | `PROV_`AE1D88                                                                                                                                                                         |
...
| **4. พฤติกรรมไฟ LED 3 (GPIO 5) ช่วงรอ vs ช่วงส่งข้อมูล** | ช่วงรอ: ไฟ LED ติดสว่างค้าง (แสดงสถานะเริ่มต้นเปิด SoftAP รอการเชื่อมต่อ) ช่วงส่ง: ไฟ LED สถานะกระพริบหรือคงสถานะติดสว่างต่อเนื่องเมื่อสมาร์ตโฟนเข้ามาเชื่อมต่อผ่าน SoftAP ทรานสปอร์ต |
...
```text
ส่งผ่านโปรโตคอล **HTTP (REST-like POST)** ไปยัง IP Address **192.168.4.1** ของ ESP32
```
...
```text
เกิด Event `WIFI_PROV_CRED_FAIL` ขึ้น โดย ESP32 จะปฏิเสธการเชื่อมต่อ แล้วเปิด SoftAP ค้างไว้เพื่อแจ้งความผิดพลาดกลับไปยังแอป และรอรับรหัสผ่านใหม่
```
2. ทำไมผู้ผลิต IoT ส่วนใหญ่จึงมองว่ากระบวนการเชื่อมต่อแบบ SoftAP มีขั้นตอนที่ยุ่งยากสำหรับผู้ใช้ทั่วไปเมื่อเทียบกับ BLE?
```text
เพราะ SoftAP บังคับให้ผู้ใช้ต้องสลับหลุดจาก Wi-Fi บ้าน เพื่อไปเกาะ Wi-Fi ของ ESP32 ก่อน ในขณะที่ BLE สามารถสแกนและส่งข้อมูลผ่านบลูทูธได้ทันทีโดยไม่ต้องสลับ Wi-Fi
```
