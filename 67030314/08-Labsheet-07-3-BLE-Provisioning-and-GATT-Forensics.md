...
| :----------------------------------------------------- | :------------------------------------------------------------------------------------------- |
| **1. BLE Device Name ที่สแกนเจอ**                      | `PROV_`1D88                                                                                  |
| **2. Primary Service UUID (128-bit)**                  | 021a9004-0382-4aea-bff4-6b3f1c5adfb4                                                         |
| **3. Characteristic Endpoint ที่พบ (0x2901)**          | 1. prov-session  <br>2. prov-config3. prov-scan (หรือเพิ่ม proto-ver, custom-data)           |
...
| **5. พฤติกรรมเมื่อต่อ Wi-Fi สำเร็จ**                   | มี (wifi_prov_scheme_ble: BT memory released, BTDM memory released)                          |
...
```text
เพราะการรับส่งข้อมูลระหว่างมือถือกับ ESP32 ทำผ่านสัญญาณ Bluetooth Low Energy (BLE) จึงไม่ต้องสลับไปเชื่อมต่อ Wi-Fi Hotspot ของอุปกรณ์ มือถือจึงใช้อินเทอร์เน็ต Wi-Fi เดิมได้ต่อเนื่อง
```
2. Descriptor `0x2901` มีความสำคัญอย่างไรต่อการที่แอปพลิเคชันมือถือจะทราบว่า Characteristic แต่ละตัวใช้ทำหน้าที่อะไร?
```text
ทำหน้าที่เก็บข้อความอธิบาย (Characteristic User Description) เช่น "prov-session" หรือ "prov-config" ทำให้แอปอ่านและแมปเข้ากับ Protocomm Endpoints ได้ถูกต้องโดยไม่ต้องสุ่มเดา
```
3. การที่ ESP-IDF มีฟังก์ชัน `esp_bt_mem_release()` มีประโยชน์อย่างไรต่อการทำงานของแอปพลิเคชัน IoT หลังเชื่อมต่อ Wi-Fi สำเร็จ?
```text
ช่วยคืนหน่วยความจำ Bluetooth Stack ทั้งหมดกลับสู่ระบบ DRAM ทำให้ ESP32 มี RAM เหลือเพิ่มขึ้นมหาศาลสำหรับประมวลผลแอปพลิเคชันหลักได้อย่างมีประสิทธิภาพ
```
